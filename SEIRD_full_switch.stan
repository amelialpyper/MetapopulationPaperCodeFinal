functions {real switch_eta(real t, real t1, real eta, real mu, real xi) {
  ...
  real[] sir(real t, real[] y, real[] theta, 
      ...
      real delta = theta[7];
      real deltaY = theta[8];
      real eta = theta[9]; // reduction in transmission rate after quarantine
      real mu = theta[10]; // shift of quarantine implementation
      real xi = theta[11]; // slope of quarantine implementation
      real eta2 = theta[12]; // reduction in transmission rate after quarantine
      real mu2 = theta[13]; // shift of quarantine implementation
      real xi2 = theta[14]; // slope of quarantine implementation
      real gamma = theta[15]; //transport related infection rate for city 1
      real gammaY = theta[16]; //transport related infection rate for city 2
      ...
      return {dS1_dt, dE1_dt, dI1_dt, dR1_dt, dD1_dt,dS2_dt, dE2_dt, dI2_dt, dR2_dt, dD2_dt}; //define our SEIRD mode for stan
  }
}
data {
  ...
  int<lower=1> N1; //population in leeds
  int<lower=1> N2; ///population in york
...
parameters {
  ...
  real<lower=0, upper=1> p_reportedY;// probability for a dead person to be reported (i.e counted as a death due to covid19)
...
generated quantities {
  //real R0Y = betaY / (deltaY + sigmaY); 
  //real R0L = betaL / (delta + sigma); //equation used to calculate R0, equation (43)
  //real ReffL[n_days];
  //real ReffY[n_days];
...
//pred_newdeaths = neg_binomial_2_rng(incidence, phi);
...
// for (i in 1:n_days)
//     ReffL[i] = switch_eta(i, tswitch, eta, mu, xi) * betaL / (gamma+sigma);
//     for (i in 1:n_days)
//     ReffY[i] = switch_eta(i, tswitch, eta2, mu2, xi2) * betaY / (gammaY+sigmaY);
    for (i in 1:n_days)
