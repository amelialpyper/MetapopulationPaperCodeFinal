functions {real switch_eta(real t, real t1, real eta, real mu, real xi) {
  ...
  real[] sir(real t, real[] y, real[] theta, 
      ...
      real beta = theta[1]; //transmission raate
      real sigma = theta[2]; //death rate
      real nu = theta[3] ; //latent period
...
transformed data {
  real x_r[1] = {tswitch};// = {gamma, nu}; //real variables used to evaluate the function, which only depend on fixed data
...
newdeaths ~ neg_binomial_2(col(to_matrix(y), 5)*p_reported, phi); //equation (6)

 
}
generated quantities {
  real R0 = beta / (delta + sigma); //equation used to calculate R0, equation (43)
  real Reff[n_days];
