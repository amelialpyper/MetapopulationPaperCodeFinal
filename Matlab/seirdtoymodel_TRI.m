...
[T Y] = ode45(@(t,y) myODEtoy(t, y), TSPAN, IC); % Solve ODE
...
ylabel('Number of Individuals in each \\ Compartment in City 1','Color', 'black',FontSize=30)
...
xlabel('Time, Days', FontSize=30); ylabel('Number of Individuals in each Compartment in City 2', FontSize=30)
...
%exportgraphics(figure(1), 'seirdtoy_leeds.pdf','Resolution',300)
%exportgraphics(figure(2), 'seirdtoy_york.pdf','Resolution',300)

function dydt = myODEtoy(t, x)
run importtraveldata.m
a=0;
w=0;
betai = 0.25; 
